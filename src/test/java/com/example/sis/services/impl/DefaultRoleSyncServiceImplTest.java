...
class DefaultRoleSyncServiceImplTest {
    ...
    @Test
    void testAutoAssignCenterRole_WithConfigCenterId() {
        // Given
        Long userId = 1L;
        Role centerRole = new Role();
        centerRole.setRoleId(2);
        centerRole.setCode("CENTER_ADMIN");
        centerRole.setName("Center Admin");
        centerRole.setActive(true);

        testUser.setDefaultCenterId(null); // User không có center
        when(authProps.getDefaultCenterId()).thenReturn(100); // Config có center

        when(authProps.isAutoAssignEnabled()).thenReturn(true);
        when(userRoleRepository.existsByUserId(userId)).thenReturn(false);
        when(userRepository.findById(1)).thenReturn(Optional.of(testUser));
        when(authProps.getDefaultRoleCode()).thenReturn("CENTER_ADMIN");
        when(roleRepository.findIdByCode("CENTER_ADMIN")).thenReturn(Optional.of(2));
        when(roleRepository.findActiveById(2)).thenReturn(Optional.of(centerRole));

        // When
        defaultRoleSyncService.ensureDefaultRoleAssigned(userId);

        // Then
        verify(userRoleService).assignIfNotExists(eq(userId), eq(2), eq(RoleScope.CENTER), eq(100));
    }

    @Test
    void testSkipAutoAssign_WhenCenterRoleButNoCenterConfigured() {
        // Given
        Long userId = 1L;
        Role centerRole = new Role();
        centerRole.setRoleId(2);
        ...
        testUser.setDefaultCenterId(null); // User không có center
        when(authProps.getDefaultCenterId()).thenReturn(null); // Config cũng không có center
        ...
        // Then
        verifyNoInteractions(userRoleService); // Không gọi assign vì không có center
    }
}